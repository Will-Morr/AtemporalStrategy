...
- Exact f64 comparisons are used for adjusted-score ties. The default penalty is none. Timing formula, fractional tie tolerance, default optional lead margin, and timed team/exhaustion interpretations remain revisable; no additional product approval is required to proceed.
...
## Draft projection, motion state, and controller outcomes

`DraftCommand` has a unique `local_id`. Its `Command<DraftItemRef>` uses tagged `persistent{id}` or `draft{local_id,item_index}` references for blueprint cancellation and pending queue-item removal. Committed `Command<String>` contains persistent IDs only. `draft::resolve_local_references` validates backward references and resolves them to canonical command-derived IDs, including factory-specific queue identities. It does not execute ticks or replace server-owned exact-state projection and ownership/capability checks. Only action assignments can initiate future removal; `draft::validate_policy` and `removal_interval` enforce the action/window shape rules.

Placement carries optional per-tile cardinal `output_directions` (N/E/S/W in lowercase on the wire). Server projection chooses missing directions in N/E/S/W order against structural legality. Blueprint and production state persist the selected direction; the output tile is never silently reselected on replay. A malformed direction count is rejected. Output feasibility, sequential metadata projection, and execution-time no-op validation are server/simulation handoff work.

Entity state includes prior committed `engaged_target`, `resolved_destination`, `failed_move_attempts`, `blocked_step`, and `born_at_tick`. These participate in hashes and checkpoints; the birth tick distinguishes new units from older units whose cooldown would otherwise allow involuntary displacement. `Displacement` presentation records preserve both participants' old/new tiles and identify the involuntary move. Combat selection, path fallback, and displacement execution belong to simulation; they must follow the current architecture table.

`timed::adjudicate` inspects exact S[new_L] for completed living build-ability entities. It persists timed-lost players independently of battlefield elimination, evaluates the whole side vector, and returns planning, finished, or history_exhausted. Active-building loss alone cannot finalize timed defeat. Team eligibility and history-exhausted/unfinished handling implement the documented revisable interpretations. A mismatch between a prior timed loss and a now-capable locked player is an error requiring examination of the finalization premise, never silently revived controller metadata.

`StopAndArchive` and `MatchArchived` carry explicit unfinished administrative metadata, separate from `Outcome`. No automatic round cap or extra award is introduced. Server must persist partial accepted turns and its last published result when archiving. `RevisionPublished.timed` and `Welcome.timed` carry optional controller timed state for replay/reconnect; `score` remains null in timed mode.
