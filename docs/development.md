...
Each match writes `replays/<match-id>/`: `manifest.json`, pinned `config.yaml`/`content.yaml`, `initial-state.json`, private `lobby.json` (slot tokens), `turns/<round>-<player>.json` plus `.request` and the original editable `.draft` (flushed before the commit is acknowledged; thinking time is persisted with each turn and round), `rounds/<round>.json` (outcome, score, timed adjudication, hashes, command outcomes, coarse timeline index, sim time; round 0 is the opening), regenerable `results/<revision>/` caches, `archive.json` after a manual stop or history exhaustion, and `measurements.jsonl`.
...
Hybrid launch: `./quickstart.sh --config config/hybrid.yaml` uses the default first-to-five scoreboard and locks 100 additional ticks per round. Hybrid preserves scoreboard eligibility rather than applying timed-mode permanent elimination; reaching the history cap without a winner ends as unfinished. While a round is still waiting, Uncommit durably removes that player’s partial input and restores their draft; published round inputs remain immutable.
