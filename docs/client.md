...
Selection priority, orders and factory loop status show a common value or Mixed; a shared bottom-panel dropdown configures eligible entities with High, Medium, Low and Off, with no battlefield priority badges. Target-building priority determines construction funding; Off constructors contribute no spending. Group labels use projected membership and appear only when assigned; the old numeric owner badges are removed. Group slots with neither members, bound factories nor saved orders are omitted from inspection.

Ore is yellow while available and gray when exhausted. Walls are darkest gray, unseen floor medium gray and seen floor lightest, on the battlefield and minimap. Miners have blocky tracked bodies in player color with a small neutral tool accent. Statistics offer Value and Slope (smoothed); the latter uses local linear regression over at most five adjacent samples without crossing null gaps, per tick (per round for thinking ratios), with a signed vertical axis.

Factory queues capture the Loop new items mode when a recipe is added. Each pending/active item shows ↻ for repeat or 1× for one-shot, with a toggle beside removal/cancellation. Consecutive matching recipe/flag pairs are grouped with counts. Shift-clicking recipe buttons adds five, for blueprints, sites and complete factories. The same controls project immediately into the draft and preserve flags through construction and replay. Turret selection shows repair rate and efficiency; the shared priority dropdown includes Off and distinguishes mixed values.
