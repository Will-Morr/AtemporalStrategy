...
MatchConfig = {
  ...
  objective: Timed{lock_ticks_per_round} | Scoreboard{
    victory_rule: FixedTarget{points} | Lead{margin}, time_penalty},
...
SimRequest = {
  ...
  minimum_end_tick  // inactivity cannot stop before this; terminal side resolution can
}
WorldState = {
  tick, last_progress_tick, inactivity_deadline, terrain, ore, players: [{bank, spend counters, eliminated, elimination_tick?, elimination_reasons?}],
...
Outcome = {
  ...
  stop_reason: elimination_resolution | inactivity | absolute_horizon,
  terminal_state_tick, last_progress_tick,
  survivors: PlayerId[], eliminated: PlayerId[],
  surviving_sides: SideId[],
  elimination_events: [{player_id, resolved_tick,
    reasons: (no_active_building | no_build_ability)[]}]
}
RoundScore = {
  entries: [{side_id, surviving_members: PlayerId[], raw_delta, adjusted_delta,
             raw_total, adjusted_total}],
  victory_rule, match_winner?: SideId
}
```
...
An elimination from resolved tick `v` appears in `S[v+1]`. Evaluate both loss predicates for every player after deaths/completions/births; record both reasons when applicable. Proposed eligibility checks only completed living entities with the corresponding capability. Individual ownership matters even in teams. Store the player/team mapping in pinned configuration, and derive hostility consistently in worker and peripheral.

Outcome classification uses the final survivor set over all original participants: empty → `draw`, all → `stalemate`, otherwise → `win`. It does not require a unique winning player. Continue after an elimination while multiple opposing sides remain; proposed early stop is zero or one surviving side. Inactivity/horizon can yield any applicable classification. Team score entries retain exact survivor membership, making count-based scoring and later audits possible. Match winners are separate from simulation outcomes.
...
Server -> Welcome{match_id, config_summary, phase, slots, fingerprint}
          ...
          MatchFinished{match_winner?: SideId, final_outcome: Outcome, reason}
```
...
Define `available_through` as the inclusive latest command tick. For terminal elimination resolution ending in `S[e]`, it is `e-1`; for inactivity ending in `S[e]`, it is `min(e, max_tick-1)`, allowing new orders to resume from the quiet endpoint. Always require `editable_from <= tick <= available_through` and `tick < max_tick`. Timed workers use `minimum_end_tick = min(max_tick, previous_editable_from + lock_ticks_per_round)` so quiet history can still advance. Once the immutable boundary reaches terminal side resolution (zero or one surviving side) or the absolute horizon, finish the match before opening another round. A quiet endpoint alone does not permanently finish the match.
