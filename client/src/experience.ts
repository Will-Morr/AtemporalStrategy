...
export class Experience {
  ...
  queue(): void {
    ...
    for(const v of views) {
      ...
      if(plan.active){const active=document.createElement('div');active.className='queue-item active';const progress=plan.active.awaiting_output?'Output blocked':`${Math.round(plan.active.paid_matter)} / ${g.types.get(plan.active.type_key)?.matter_cost} matter`;active.append(unitIcon(plan.active.type_key,g.color(v.owner)),document.createTextNode(`Building ${plan.active.type_key} · ${progress}`));const cancel=button('×',()=>g.stage({kind:'edit_production',factories:[v.id],edit:{kind:'cancel_active'}}));cancel.setAttribute('aria-label',`Cancel active ${plan.active.type_key}`);cancel.title='Cancel this unit; spent matter is lost';cancel.onfocus=()=>{this.focusDelete=()=>cancel.click();};cancel.disabled=!enabled;active.append(cancel);queue.append(active);}
