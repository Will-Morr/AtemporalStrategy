...
export function factoryPlan(g: Game, v: EntityView) {
  ...
  let pending: {type: string; ref: DraftItemRef2 | null}[]=v.blueprint ? (v.settings?.queue??[]).map(type=>({type,ref:null})) : (p?.pending_items??[]).map(i=>({type:i.type_key,ref:{kind:'persistent',id:i.item_id}}));
    ...
    if('factories' in c && c.factories.some(id=>same(id,v.id))) {
      ...
      if(c.kind==='edit_production') {
        ...
        if(e.kind==='append'||e.kind==='replace_pending') {
          const items=e.items.map((type,item_index)=>({type,ref:{kind:'draft' as const,local_id:d.local_id,item_index}}));
