...
assert.equal(report.stats.unexpected, 5);
...
for (const spec of specs) {
  ...
  assert(!logs.some(item => ['capture-error', 'cleanup-error'].includes(item.kind)), `${spec.title}: evidence collection failed`);
  if (spec.title.includes('console errors')) assert(logs.some(item => item.level === 'error' && item.text.includes('intentional consoleerror probe')));
  ...
  if (spec.title.includes('each identity')) {
    ...
    const closedVideo = result.attachments.find(item => item.name === 'spectator-video-1');
    assert(closedVideo, 'closed spectator page must retain its video');
    assert((await stat(closedVideo.path)).size > 0);
...
console.log(`Harness failure checks passed: five intentional failures, diagnostics, screenshots, traces and identity videos verified.\nArtifacts: ${artifacts}`);
