...
await rm(`${dir}/dense`,{recursive:true,force:true});
const server=await startServer(dir,'dense',['--config',`${dir}/config.json`,'--content',`${dir}/content.json`,'--memory-budget-mb','256','--results-budget-mb','512']);
const summary={machine:{cpu:cpus()[0].model,logical_cpus:cpus().length,ram_bytes:totalmem(),platform:platform()},config:config.match_defaults,content_changes:'weapons disabled; factory and grunt cost 1; two queues of 1000 grunts',rounds:[],seeks:[],peak_rss_kib:0};
...
try{
  ...
  await page.screenshot({path:`${dir}/dense-battlefield.png`});await context.tracing.stop({path:`${dir}/trace.zip`});assert(errors.length===0,`browser errors: ${errors}`);
